...
(() => {
  ...
  const handlers = {
    ...
    "shell:cwd-change": (p) => {
      cwd = p?.cwd ?? "";
      // Refresh files panel if open (directory may have changed)
  ...
  const fetchFiles = async () => {
    ...
    // Clear existing entries (keep the empty placeholder)
  ...
  const renderFiles = (files) => {
    ...
    // Remove old entries (keep .files-empty)
    filesBody.querySelectorAll(".files-entry").forEach((el) => el.remove());
    if (files.length === 0) {
  ...
  filesToggle?.addEventListener("click", () => {
    setFilesOpen(filesPanel.hasAttribute("hidden"));
  });
